...
def _get_current_interview_credits(data: Dict[str, Any]) -> int:
    try:
        return int(data.get("interviewCredits") or 0)
    except (TypeError, ValueError):
        return 0


def consume_interview_credits(uid: str, amount: int = 1) -> int:
    """Atomically decrement interview credits; returns remaining balance."""
    ...
    def _consume(transaction: firestore.Transaction) -> int:
        ...
        if not snapshot.exists:
            raise ValueError("User not found")
        data = snapshot.to_dict() or {}
        current = _get_current_interview_credits(data)
        if current < amount:
            raise InsufficientCreditsError("Insufficient interview credits")
        remaining = current - amount
        transaction.update(user_ref, {"interviewCredits": remaining})
        return remaining

    return _consume(db.transaction())


def grant_interview_credits(
    uid: str,
    amount: int,
    *,
    ...
    """Atomically add interview credits; returns updated balance."""
    ...
    def _grant(transaction: firestore.Transaction) -> int:
        ...
        transaction.update(user_ref, {"interviewCredits": updated})
        return updated
        ...
        log_ref.set(
            {
                "uid": uid,
                "amount": amount,
                "reason": reason or "interview_pack",
